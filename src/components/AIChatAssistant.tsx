...
// Reusable gold logo badge used everywhere in the chatbot
function RevoLogoBadge({ size = 40, className = '' }: { size?: number; className?: string }) {
  return (
    <div
      className={`shrink-0 rounded-xl bg-gradient-to-br from-amber-300 via-yellow-400 to-amber-500 flex items-center justify-center shadow-md shadow-amber-500/40 ring-1 ring-amber-600/40 ${className}`}
...
export function AIChatAssistant({ onClose }: Props) {
  ...
  const updateSessionLocal = (id: string, patch: Partial<ChatSession>) => {
    setSessions((prev) =>
  ...
  const sendMessage = async (text?: string) => {
    ...
    let title = active?.title ?? 'محادثة جديدة';
    ...
    if (!sessionId) {
      ...
      title = content.slice(0, 30);
      updateSessionLocal(sessionId, { title });
    }

    const userMsg: ChatMessage = { role: 'user', content };
    const placeholder: ChatMessage = { role: 'assistant', content: '' };
    let runningMessages = [...workingMessages, userMsg, placeholder];
    setSessions((prev) =>
      prev.map((s) => (s.id === sessionId ? { ...s, title, messages: runningMessages, updatedAt: Date.now() } : s))
    );
    ...
    try {
      ...
      if (!accessToken) throw new Error('سجّل دخولك أولاً للاستخدام.');
      ...
      if (!resp.ok) {
        ...
        if (resp.status === 403 && body.code === 'no_quota') throw new Error('انتهت محاولاتك المجانية.');
        ...
        while ((nl = buf.indexOf('\n')) !== -1) {
          ...
          try {
            ...
            if (delta) {
              acc += delta;
              runningMessages = [
                ...runningMessages.slice(0, -1),
                { role: 'assistant', content: acc },
              ];
              setSessions((prev) =>
                prev.map((s) =>
                  s.id === sessionId ? { ...s, messages: runningMessages, updatedAt: Date.now() } : s
                )
              );
            }
          } catch {
      ...
      if (e?.name === 'AbortError') {
        runningMessages = [
          ...runningMessages.slice(0, -1),
          { role: 'assistant', content: (runningMessages[runningMessages.length - 1]?.content ?? '') + '\n\n_— تم إلغاء العملية._' },
        ];
        setSessions((prev) =>
          prev.map((s) => (s.id === sessionId ? { ...s, messages: runningMessages } : s))
        );
        persistSession({ id: sessionId!, title, messages: runningMessages, updatedAt: Date.now() });
      } else {
    ...
    >
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-200 dark:border-slate-800 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={startNew}
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-br from-amber-400 to-yellow-500 hover:from-amber-500 hover:to-yellow-600 text-amber-950 text-xs font-bold transition-colors shadow-md shadow-amber-500/30"
              title="محادثة جديدة"
            >
            ...
            >
              ...
              {sessions.length > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-amber-500 text-white text-[10px] flex items-center justify-center rounded-full">
                  {sessions.length}
                </span>
              )}
            </button>
          ...
          <>
            ...
            >
              ...
              <div className="p-3 border-t border-gray-200 dark:border-slate-800">
                <button
                  onClick={startNew}
                  className="w-full py-2 rounded-lg bg-gradient-to-br from-amber-400 to-yellow-500 hover:from-amber-500 hover:to-yellow-600 text-amber-950 text-sm font-bold flex items-center justify-center gap-2"
                >
          ...
          {messages.length === 0 ? (
            ...
            >
              <motion.div
                ...
                className="mx-auto"
              >
                <RevoLogoBadge size={96} className="!rounded-3xl shadow-2xl shadow-amber-500/40" />
              </motion.div>
              <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl mx-auto pt-4">
                {SUGGESTIONS.map((s, i) => (
                  <motion.button
                    ...
                    className="p-4 text-right text-sm rounded-xl bg-white dark:bg-slate-800/60 border border-gray-200 dark:border-slate-700 hover:border-amber-400 dark:hover:border-amber-500 hover:shadow-md transition-all text-gray-700 dark:text-slate-200 flex items-center gap-3"
                  >
            ...
            messages.map((m, idx) => (
              ...
              >
                {m.role === 'assistant' && <RevoLogoBadge size={36} />}
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                    m.role === 'user'
                      ? 'bg-gradient-to-br from-amber-400 to-yellow-500 text-amber-950 rounded-tr-sm shadow-md shadow-amber-500/30'
      ...
      <footer className="border-t border-gray-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 backdrop-blur">
        <div className="max-w-3xl mx-auto px-4 py-4">
          ...
          <div className="relative flex items-end gap-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl shadow-sm focus-within:ring-2 focus-within:ring-amber-400 transition-all">
            <textarea
              value={input}
            ...
            <button
              ...
              className="m-1.5 p-2.5 rounded-xl bg-gradient-to-br from-amber-400 to-yellow-500 hover:from-amber-500 hover:to-yellow-600 text-amber-950 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-md shadow-amber-500/30 shrink-0"
              title="إرسال"
            >
          ...
          <p className="text-[10px] text-gray-400 text-center mt-2">
            Revo Teacher مخصص للمواد الدراسية فقط 📚 • Enter للإرسال
          </p>
        </div>
      </footer>
    </motion.div>
    ...
    <div className="flex items-center gap-3 py-1">
      <div className="flex gap-1">
        {[0, 1, 2].map((i) => (
          <motion.span
            key={i}
            className="block w-2 h-2 rounded-full bg-gradient-to-br from-amber-400 to-yellow-500"
            animate={{
              y: [0, -6, 0],
              opacity: [0.4, 1, 0.4],
              scale: [0.8, 1.2, 0.8],
